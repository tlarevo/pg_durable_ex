...
PROJECT="pg-durable-ex-test-pg17"
COMPOSE_FILE="docker/docker-compose.pg17.yml"
PORT="${PG_DURABLE_PG17_PORT:-55417}"

cleanup() {
  echo "Cleaning up PG17 test environment (project: ${PROJECT})..."
  docker compose -f "${COMPOSE_FILE}" --project-name "${PROJECT}" down -v 2>/dev/null || true
}
trap cleanup EXIT

echo "=== Starting PostgreSQL 17 with pg_durable v0.2.7 (port ${PORT}) ==="
docker compose -f "${COMPOSE_FILE}" --project-name "${PROJECT}" down -v 2>/dev/null || true
...
CONTAINER="${PROJECT}-pg_durable_pg17-1"

echo "Waiting for PostgreSQL 17 on port ${PORT}..."
...
echo "=== Running integration tests (PG17, port ${PORT}) ==="
